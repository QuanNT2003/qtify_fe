...
export function CategoryCard({ title, color }: CategoryCardProps) {
  return (
    <Link
      href={`/search?category=${encodeURIComponent(title)}`}
      className="group relative aspect-3/2 overflow-hidden rounded-lg p-4 transition-transform hover:scale-[1.02] cursor-pointer"
